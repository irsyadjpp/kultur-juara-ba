      ...
      <form className="space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-8">
            ...
            {/* 3.1. KOMPOSISI TUBUH DETAILED (Moved from Reg) */}
            <SectionCard title="Komposisi Tubuh & Vitals" icon={Weight} description="Pengukuran InBody/Tanita dan Tanda Vital.">
              <div className="space-y-6">
                {/* Lingkar */}
                <div className="grid grid-cols-3 gap-4">
                  <MetricInput label="Lingkar Lengan" unit="cm" />
                  <MetricInput label="Lingkar Paha" unit="cm" />
                  <MetricInput label="Lingkar Betis" unit="cm" />
                </div>
                {/* Vital */}
                <div className="grid grid-cols-3 gap-4">
                  <MetricInput label="Tekanan Darah (Sys)" unit="mmHg" />
                  <MetricInput label="Tekanan Darah (Dia)" unit="mmHg" />
                  <MetricInput label="Denyut Nadi" unit="bpm" />
                </div>
                {/* Body Comp */}
                <div className="grid grid-cols-2 gap-4">
                  <MetricInput label="Body Fat" unit="%" />
                  <MetricInput label="Skeletal Muscle" unit="%" />
                  <MetricInput label="Muscle Mass" unit="%" />
                  <MetricInput label="Body Water" unit="%" />
                  <MetricInput label="Bone Mass" unit="%" />
                  <MetricInput label="Visceral Fat" unit="idx" />
                  <MetricInput label="BMR" unit="kcal" />
                  <MetricInput label="AMR" unit="kcal" />
                  <div className="col-span-2 space-y-2">
                    <Label>Somatotype (Tipe Tubuh)</Label>
                    <Select>
                      <SelectTrigger className="h-12 rounded-xl bg-secondary border"><SelectValue placeholder="Pilih Tipe Tubuh" /></SelectTrigger>
                      <SelectContent>
